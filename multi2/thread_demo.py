...
def threadBoth(source=0):
    ...
    video_getter = VideoGet(source).start()
    video_filter = VideoFilter(video_getter.grabbed,video_getter.frame).start()
    video_shower = VideoShow(video_filter.frame).start()

    while True:
        if video_getter.stopped or video_shower.stopped or video_filter.stopped:
            video_shower.stop()
            video_filter.stop()
            video_getter.stop()
            break
        ...
        video_filter.grabbed = grabbed
        video_filter.frame = frame

        video_shower.frame = video_filter.frame
